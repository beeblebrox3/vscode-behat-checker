 ...
 */
class BehatStepsParser {
  constructor(projectDirectory, configFile = "behat.yml", behatPath = "vendor/bin/behat") {
    this.steps = [];
    this.updateConfig(projectDirectory, configFile, behatPath);
    this.updateStepsCache();
  }

  treatAssetPath(projectDir, assetPath) {
    return path.isAbsolute(assetPath) ?
      path.resolve(assetPath) :
      path.resolve(projectDir, assetPath);
  }
   ...
   * @param {String} behatPath
   *
   * @memberOf BehatStepsParser
   */
  updateConfig(projectDirectory, configFile, behatPath = "vendor/bin/behat") {
  ...
  getBehatPath() {
    ...
    return path.resolve(this.projectDirectory, composerBinDir, "behat");
  }
